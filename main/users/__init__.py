...
from main.models import db, Users, UserSchema
...
serializer = Serializer('AKDBbsfbsd8', 600)
...
class UsersEndpoint(Resource):
    ...
    def post(self):
        ...
        return {
            'response': "User data should not be empty"
        }
        ...
        if user_data:
            try:
                ...
                if not target_user or not target_user.verify(user_data.get('password')):
                    return {
                        'response': 'Invalid username or password. Try again'
                    }
